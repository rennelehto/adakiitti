...
def kentän_etäisyys(py):
    ...
    tulos = kursori.fetchall()
    return tulos
#Hakee kentät joille voi matkustaa
...
#Hakee ICAO-koodilla kentän nimen
...
def peli_alkaa():
    ...
    lore1 = f" Hei {pelaajan_nimi}! Muinaiset tietäjälahkot ovat sodassa! Vanhat lahkot, joiden tavoite on ilmastonmuutos,"
    ...
    input('')
    lore2 = " Uudet lahkot ovat päättäneet pysäyttää heidän aikeensa"
    ...
    pelin_aloitus = input(                          #Vastaus valinta
                ...
                "\n: ")
    while pelin_aloitus == (""):
        aloitusvastaus = 'Virhe, yritä uudestaan.'
        pelin_aloitus = input(  # Vastaus valinta
            "\n1. Asia selvä. "
            "\n2. En halua. "
            "\n: ")
    while int(pelin_aloitus) in [1,2]:
        if int(pelin_aloitus) == 1:
            aloitusvastaus = 'Asia selvä!'
            break
        if int(pelin_aloitus) == 2:
            aloistusvastaus = 'Valitettavasti et saa jatkaa.'
            quit()
        if int(pelin_aloitus) not in [1,2]:
            aloitusvastaus = 'Yritä uudelleen.'
            pelin_aloitus = input(  # Vastaus valinta
                "\n1. Asia selvä. "
                "\n2. En halua. "
                "\n: ")
...
def loppu():
    ...
    pojot = 0
    while ö < p.kivet:
        pojot = pojot + (random.randint(1, 6))
        ö = ö + 1
    return pojot
#PÄÄOHJELMA
