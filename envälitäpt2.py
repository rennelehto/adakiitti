...
def kenttäkyselyEU(): #hakee Euroopan isot kentät
...
def matkustettavat_kentät_pahis():
    ...
    while k <= 20: #Hakee vain ensimmäiset 20 kenttää
        ...
        k = k + 1
    return
def pahiksen_koordinaatit(pahiksen_sijainti_icao):
    sql = f"SELECT latitude_deg, longitude_deg FROM airport WHERE ident = '{pahiksen_sijainti_icao}'"
    kursori = yhteys.cursor()
    kursori.execute(sql)
    tulos = kursori.fetchall()
    return tulos
...
def koodi_nimeksi_hyvis(koodi):
    ...
    if tulos:
        for rivi in tulos:
            if rivi not in seuraavien_kenttien_nimet_hyvis:
...
def peliluuppi1():
    ...
    print('Löysit adakiitin jonka arvo on 5!')
    print()
...
def loppumähinä(hyvis, pahis):
    if hyvis != pahis:
        if hyvis > pahis:
            print('Voitit pelin ja autoit maailmaa parantumaan')
        if pahis > hyvis:
            print('Voi ei, pahis pähitti sinut!')
    if hyvis == pahis:
        print('Tasapeli! Viimeisillä voimillanne yrittätte antaa toisillenne pataan...')
        if hyvis + random.randint(0, 6) > pahis + random.randint(0, 6):
            print('Hahaa sait tökkäistyä velhoa silmään! Olet voittaja')





loppumähinä(kerätyt_kivet_hyvis,kerätyt_kivet_pahis)
